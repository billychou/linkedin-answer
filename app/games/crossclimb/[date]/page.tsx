...
export async function generateStaticParams() {
  const answers = getAllAnswers("crossclimb");
...
export async function generateMetadata({
  ...
  const game = getGame("crossclimb");
  const answer = getAnswerByDate("crossclimb", date);
  ...
  return constructMetadata({
    ...
    path: `/games/crossclimb/${date}`,
    canonicalUrl: `/games/crossclimb/${date}`,
  });
}

export default async function CrossclimbDatePage({ params }: Props) {
  const { date } = await params;
  const game = getGame("crossclimb");
  const answer = getAnswerByDate("crossclimb", date);
