...
export async function generateStaticParams() {
  const answers = getAllAnswers("pinpoint");
...
export async function generateMetadata({
  ...
  const answer = getAnswerByDate("pinpoint", date);
  ...
  return constructMetadata({
    ...
    path: `/games/pinpoint/${date}`,
    canonicalUrl: `/games/pinpoint/${date}`,
  });
}

export default async function PinpointDatePage({ params }: Props) {
  const { date } = await params;
  const game = getGame("pinpoint");
  const answer = getAnswerByDate("pinpoint", date);
