...
export async function generateStaticParams() {
  const answers = getAllAnswers("tango");
...
export async function generateMetadata({
  ...
  const game = getGame("tango");
  const answer = getAnswerByDate("tango", date);
  ...
  return constructMetadata({
    ...
    path: `/games/tango/${date}`,
    canonicalUrl: `/games/tango/${date}`,
  });
}

export default async function TangoDatePage({ params }: Props) {
  const { date } = await params;
  const game = getGame("tango");
  const answer = getAnswerByDate("tango", date);
