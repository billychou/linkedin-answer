...
export async function generateStaticParams() {
  const answers = getAllAnswers("mini-sudoku");
...
export async function generateMetadata({
  ...
  const game = getGame("mini-sudoku");
  const answer = getAnswerByDate("mini-sudoku", date);
  ...
  return constructMetadata({
    ...
    path: `/games/mini-sudoku/${date}`,
    canonicalUrl: `/games/mini-sudoku/${date}`,
  });
}

export default async function MiniSudokuDatePage({ params }: Props) {
  const { date } = await params;
  const game = getGame("mini-sudoku");
  const answer = getAnswerByDate("mini-sudoku", date);
